...
public class NetExpection {
    ...
    private static void onException(ExceptionReason reason) {
        switch (reason) {
            case CONNECT_ERROR:
                CustomToast.showToast(CONNECT_ERROR);
                break;

            case CONNECT_TIMEOUT:
                CustomToast.showToast(CONNECT_TIMEOUT);
                break;

            case BAD_NETWORK:
                CustomToast.showToast(BAD_NETWORK);
                break;
            case PARSE_ERROR:
                CustomToast.showToast(PARSE_ERROR);
                break;
            case UNKNOWN_ERROR:
            default:
                CustomToast.showToast(UNKNOWN_ERROR);
                break;
        }
    }
